...
// ⬇️ Importa los schemas externos
import {
  makeTreatmentSchemas,
  getFormSchemaPair,
  zodToErrors,
...
// -------- Shared form (usa schemas externos) --------
function MetaForm<T extends Priority | Treatment>({
  ...
  // Inyecta verificación runtime de existencia de íconos
  const treatmentSchemas = useMemo(
    () =>
      makeTreatmentSchemas((key) => {
        return !!getIconComponent(key);
      }),
    []
  );

  function validateWithSchema(): { ok: true; data: any } | { ok: false; e: Record<string, string> } {
    const isEdit = !!(item as any)?._id;
    const schema = getFormSchemaPair(kind, isEdit, treatmentSchemas);
    ...
    if (isPriority) {
      ...
      if (!parsed.success) return { ok: false, e: zodToErrors(parsed.error.issues) };
  ...
  function submit() {
    const res = validateWithSchema();
    if (!res.ok) {
      setErrors(res.e);
      return;
    }
    setErrors({});
    onSave(res.data); // envía solo campos validados (strip aplicado)
  }
            ...
            return (
              <Tr key={row._id || row.name} _hover={{ bg: "blackAlpha.50" }}>
                <Td>
                  <HStack>
                    {MainIcon ? (
                      <Icon as={MainIcon} color={row.color} boxSize={5} />
                    ) : (
                ...
                <Td>
                  <VStack align="start" spacing={1}>
                    ...
                    <HStack>
                      {MiniIcon ? (
                        <Icon as={MiniIcon} color={row.color} boxSize={4} />
                      ) : (
                        <Text fontSize="sm">—</Text>
                      )}
                      <Text fontSize="xs" color="gray.600">
