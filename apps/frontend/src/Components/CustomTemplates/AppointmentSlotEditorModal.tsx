                ...
                <Collapse in={isExpanded} animateOpacity>
                  <VStack align="stretch" spacing={4} mt={3}>
                    ...
                    <VStack align="stretch" spacing={2}>
                      <FormLabel fontSize="sm">⏱️ Duration (minutes)</FormLabel>
                      <HStack spacing={2}>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => {
                            const currentDuration = slot.duration || globalDuration || 60;
                            const newDuration = Math.max(5, currentDuration - 15);
                            const newStartDate = dayjs(slot.startDate).tz(tz);
                            const newEndDate = newStartDate.add(newDuration, 'minute').toDate();
                            onSlotChange(idx, {
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const newDuration = Math.max(5, parseInt(e.target.value) || 60);
                            const newStartDate = dayjs(slot.startDate).tz(tz);
                            const newEndDate = newStartDate.add(newDuration, 'minute').toDate();
                            onSlotChange(idx, { 
                              duration: newDuration,
